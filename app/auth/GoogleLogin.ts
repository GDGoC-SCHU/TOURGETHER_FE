...
// 구글 인증 URL
const GOOGLE_AUTH_URL = `${API_URL}/oauth2/authorization/google`;
...
/**
 * 웹 환경에서 현재 창에서 직접 리다이렉트 사용하는 구글 로그인
 */
function webGoogleLoginDirect(): Promise<{
  token: string;
  refreshToken: string | undefined;
  user: any
}> {
  return new Promise((resolve, reject) => {
    try {
      console.log("직접 리다이렉트로 구글 로그인 시작");
      ...
      if (accessToken && userId) {
        console.log("URL에서 구글 로그인 정보 감지됨", {
          accessToken,
          refreshToken,
          userId,
      ...
      console.log("구글 인증 페이지로 리다이렉트:", redirectUrl);
...
/**
 * 네이티브 환경에서 WebBrowser를 사용하는 구글 로그인
 */
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((result) => {
      ...
      if (result.type !== "success" && result.type !== "dismiss") {
        subscription.remove();
        reject(new Error("구글 인증이 취소되었거나 실패했습니다"));
      }
    })
    .catch((error) => {
